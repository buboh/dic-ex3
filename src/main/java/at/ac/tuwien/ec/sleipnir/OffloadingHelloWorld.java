...
public class OffloadingHelloWorld {
	...
	public static void main(String[] arg) {
		// + parse config file and save parsed params into "OffloadingSetup"
		...
		// same as in OffloadingMain
		processArgs(arg);
		Logger.getLogger("org").setLevel(Level.OFF);
		Logger.getLogger("akka").setLevel(Level.OFF);

		// Class used to compare deployments according to frequency, i.e., the number of
		// times they appear in the histogram
		class FrequencyComparator implements Serializable,
			...
			public int compare(Tuple2<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>> o1,
				...
				/*
				 * Deployments are characterized by a Tuple2, whose values are a
				 * OffloadScheduling and a Tuple5 containing all its execution parameters
				 * (frequency, runtime, cost, battery lifetime, execution time); since we are
				 * interested in the frequency, we select the Tuple5 ( o1._2() ) and we pick its
				 * first value ( _1() )
				 */
		...
		configuration.setAppName("Sleipnir-ML");
		...
		ArrayList<Tuple2<MobileApplication, MobileCloudInfrastructure>> inputSamples = generateSamples(
				OffloadingSetup.iterations);
		...
		if (!outFile.exists()) {
			...
			try {
				...
				Tuple2<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>> mostFrequent = histogram
						.max(new FrequencyComparator());
				/*
				 * By default, schedulings are saved in the file as a single string, where each
				 * value is separated by \t. values are: 1) the description of the scheduling,
				 * with t -> n indicating that task t has been scheduled to node n; 2) the
				 * frequency of the deployment, meaning the number of times where the deployment
				 * occurs in the histrogram; 3) the deployment runtime 4) the user cost of the
	 ...
	 * @param algoritmName the name of scheduling algorithm, can be used to decide
	 *                     at runtime which algorithm you want to call
	 * @return
	 */
	private static JavaPairRDD<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>> runSparkSimulation(
			JavaSparkContext jscontext, ArrayList<Tuple2<MobileApplication, MobileCloudInfrastructure>> inputSamples,
			String algoritmName) {
		JavaRDD<Tuple2<MobileApplication, MobileCloudInfrastructure>> input = jscontext.parallelize(inputSamples);

		JavaPairRDD<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>> results = input.flatMapToPair(
				new PairFlatMapFunction<Tuple2<MobileApplication, MobileCloudInfrastructure>, OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>>() {
					...
					public Iterator<Tuple2<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>>> call(
						...
						ArrayList<OffloadScheduling> offloads = (ArrayList<OffloadScheduling>) singleSearch
								.findScheduling();

						if (offloads != null) {
							for (OffloadScheduling os : offloads) {
								/*
								 * By default, schedulings are saved in a Tuple2, whose values are a
								 * OffloadScheduling object and a Tuple5 containing values of scheduling
								 * execution. The Tuple5 values are: 1) the frequency of the deployment, meaning
								 * the number of times where the deployment occurs in the histrogram. We set it
								 * to 1 and aggregate later; 2) the deployment runtime 3) the user cost of the
								 * deployment 4) the battery lifetime of the deployment 5) the execution time of
								 * the algorithm
								 */
								output.add(
										new Tuple2<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>>(
												os,
												new Tuple5<Integer, Double, Double, Double, Double>(1, os.getRunTime(),
														os.getUserCost(), os.getBatteryLifetime(),
														os.getExecutionTime())));
							}
						}
		...
		JavaPairRDD<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>> aggregation = results
				.reduceByKey(
						new Function2<Tuple5<Integer, Double, Double, Double, Double>, Tuple5<Integer, Double, Double, Double, Double>, Tuple5<Integer, Double, Double, Double, Double>>() {
							/**
							 * 
							 */
							private static final long serialVersionUID = 1L;
							...
							public Tuple5<Integer, Double, Double, Double, Double> call(
								...
								return new Tuple5<Integer, Double, Double, Double, Double>(off1._1() + off2._1(),
										off1._2() + off2._2(), off1._3() + off2._3(), off1._4() + off2._4(),
										off1._5() + off2._5());
							}

						});

		JavaPairRDD<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>> histogram = aggregation
				.mapToPair(
						new PairFunction<Tuple2<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>>, OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>>() {
							private static final long serialVersionUID = 1L;
							...
							public Tuple2<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>> call(
								...
								/*
								 * We aggregate similar scheduling according to their average, therefore each
								 * value is averaged according to the frequency ( val._1() ). Execution time of
								 * the algorithm is instead averaged by the number of iterations.
								 */
								Tuple5<Integer, Double, Double, Double, Double> tNew = new Tuple5<Integer, Double, Double, Double, Double>(
										val._1(), val._2() / val._1(), val._3() / val._1(), val._4() / val._1(),
										(val._5() / OffloadingSetup.iterations));
								return new Tuple2<OffloadScheduling, Tuple5<Integer, Double, Double, Double, Double>>(
										arg0._1, tNew);
							}
						});
		return histogram;
	...
	// + Sampling Phase - generates a number of application-infrastructure pairs for
	// simulation
	private static ArrayList<Tuple2<MobileApplication, MobileCloudInfrastructure>> generateSamples(int iterations) {
		...
		for (int i = 0; i < iterations; i++) {
			...
			// + generates a workload consisting of randomly selected apps
			WorkloadGenerator generator = new WorkloadGenerator();
			// For each mobile, we generate a different workload, and aggregate them into a
			// global workload
			...
			// This object models the sampled infrastructure; from now on, it is modified
			...
			// + this is not an object, this is a class
			DefaultCloudPlanner.setupCloudNodes(inf, OffloadingSetup.cloudNum);
			// We set up edge nodes according to EdgePlanner object (in this case, we place
			// a edge node per cell).
			// + this is not an object, this is a class
			...
			Tuple2<MobileApplication, MobileCloudInfrastructure> singleSample = new Tuple2<MobileApplication, MobileCloudInfrastructure>(
					globalWorkload, inf);
