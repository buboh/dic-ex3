...
/**
 * OffloadScheduler class that implements the Heterogeneous Earliest-Finish-Time
 * (HEFT) algorithm , a static scheduling heuristic, for efficient application
 * scheduling
 *
 * H. Topcuoglu, S. Hariri and Min-You Wu, "Performance-effective and
 * low-complexity task scheduling for heterogeneous computing," in IEEE
 * Transactions on Parallel and Distributed Systems, vol. 13, no. 3, pp.
 * 260-274, March 2002, doi: 10.1109/71.993206.
 */
...
public class HLFETScheduler extends OffloadScheduler {
	...
	/**
	 * Processor selection phase: select the tasks in order of their priorities and
	 * schedule them on its "best" processor, which minimizes task's executuion!
	 * time
	 * 
	 * @return
	 */
	...
	public ArrayList<? extends OffloadScheduling> findScheduling() {
		 ...
		 * Once nodes are scheduled, they are taken from the PriorityQueue according to
		 * their runtime
		 */
		PriorityQueue<MobileSoftwareComponent> scheduledNodes = new PriorityQueue<MobileSoftwareComponent>(
				new RuntimeComparator());
		/*
		 * tasks contains tasks that have to be scheduled for execution. Tasks are
		 * selected according to their calculateBLevel (at least in HEFT)
		 */
		PriorityQueue<MobileSoftwareComponent> tasks = new PriorityQueue<MobileSoftwareComponent>(
		...
		while ((currTask = tasks.poll()) != null) {
			// If there are nodes to be scheduled, we check the first task who terminates
			...
			if (!currTask.isOffloadable()) {
				...
				if (isValid(scheduling, currTask,
			...
			// if scheduling found a target node for the task, it allocates it to the target
			// node
			if (target != null) {
	...
	private double calculateBLevel(MobileSoftwareComponent msc,
		 ...
		 */
		{
			...
			for (ComponentLink neigh : dag.outgoingEdgesOf(msc)) // for the exit task rank=w_cmp
			{
				...
				thisNeighbor = calculateBLevel(neigh.getTarget(), dag, infrastructure); // succesor's comuptational
																						// weight (= Node Weight)
